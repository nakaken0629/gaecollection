...
function loadHistory() {
    ...
    $.get("/history", function(data) {
        $.each(data, function(i, history) {
            $("#history ul").append("<li><a href='#' onclick='loadPicture(" + history.id + "); return false;'>" + history.createDate + "</a></li>");
        })
    });
}
...
$(window).load(function() {
    ...
    $("#mycanvas").mousedown(mouseDownHandler).mousemove(
            mouseMoveHandler).mouseup(mouseUpOrOutHandler).mouseout(
            mouseUpOrOutHandler);
    /* 用意する履歴キャンバスは2次元用で、太さ4の灰色の線とする */
    historycontext = document.getElementById("historycanvas")
            .getContext("2d");
...
/* マウスボタンが放された、あるいはマウスカーソルがキャンバスをはみ出したときの処理 */
function mouseUpOrOutHandler(e) {
    ...
    $.ajax({
        type : "POST",
        url : "/addstroke",
        data : {"clientId" : clientId, "stroke" : JSON.stringify(points) }
    });
}
